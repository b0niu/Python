...
def access_shop():
    ...
    try:
        with connection.cursor() as cursor:
            sql = " SELECT ID_Towar, Kod_modelu, Typ, Cena, Rozmiar FROM Towar t \
                    INNER JOIN Magazyn m ON t.ID_Magazyn = m.ID_Magazyn \
                    WHERE m.Nr_Magazynu = 1 "
            cursor.execute(sql)
            rows = cursor.fetchall()
            for row in rows:
                print(row["ID_Towar"], row["Kod_modelu"], row["Typ"], row["Cena"])
    finally:
        connection.close()
...
def buy_product(date, worker, client):
    connection = connect()
    try:
        with connection.cursor() as cursor:

            sql = ("SELECT count(ID_Sprzedaz) ID FROM Sprzedaz ")
            cursor.execute(sql)
            rows = cursor.fetchone()
            id = int(rows["ID"])
            id+=1

            sql1 = ("INSERT INTO Sprzedaz VALUES (%s,'%s',%s,%s)") % (id,date,worker,client)
            cursor.execute(sql1)
            connection.commit()
            i = 0
            zakup = 1
            ...
            while (zakup):
                access_shop()
                if (i != 0):
                    zakup = int(input("Czy chcesz kontynowac zakupy? Tak = 1, Nie != 1 "))
                    if(zakup != 1):
                        break

                id_p = input("Podaj ID towaru")
                sql2= ("INSERT INTO Rachunek VALUES (%s,%s)") % (id_p,id)
                cursor.execute(sql2)
                i+=1
                sql3 = ("UPDATE Towar SET ID_Magazyn = 0 Where ID_Towar = %s")% (id_p)
                cursor.execute(sql3)

                connection.commit()
            if(i==0):
                sql4= ("DELETE FROM Sprzedaz WHERE ID_Sprzedaz = %s") % (id)
                cursor.execute(sql4)
                connection.commit()
    finally:
        connection.close()